...
class Deep_Q_learning:
    ...
    def __init__(self, maze, loss, hidden_size, learning_rate, memory_size):
        ...
        # self.Qmodel.add(Dense(hidden_size, activation="relu", input_dim=self.state_dim))
        # self.Qmodel.add(Dense(hidden_size, activation="relu"))
        # self.Qmodel.add(Dense(self.num_action, activation="linear"))
        # self.Qmodel.compile(loss=loss, optimizer=Adam(lr=learning_rate))
        # メモリ
        self.memory = Memory(memory_size)

    def update(self, batch_size, discount_rate):
        if self.memory.len() < batch_size:
            return
        mini_batch = self.memory.sample(batch_size)

        # 状態を入力として
        inputs = np.array([state for (state, _, _, _) in mini_batch])
        # 基本はQネットワークの出力をそのまま予測する
        # targets = self.Qmodel.predict(inputs)
        targets = self.Qmain.predict(inputs)

        actions = np.array([action for (_, action, _, _) in mini_batch])
        rewards = np.array([reward for (_, _, reward, _) in mini_batch])

        next_states = np.array([next_state for (_, _, _, next_state) in mini_batch])
        # pred_max = self.Qmodel.predict(next_states).max(axis=1)
        next_actions = self.Qmain.predict(next_states).argmax(axis=1)
        # pred_max = self.Qtarget.predict(next_states).max(axis=1)
        ...
        targets[np.arange(batch_size), actions] = rewards + discount_rate * pred_max

        # self.Qmodel.train_on_batch(inputs, targets)
    ...
    def select_best_action(self):
        """評価値の最も高い行動を探す"""
        # pred = self.Qmodel.predict([self.state])[0]
    ...
    def step(self, discount_rate, random_rate, batch_size):
        ...
        # Qネットワークの重みを更新する
        # self.update(batch_size, discount_rate)
